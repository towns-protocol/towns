...
import { generateAlchemyRpcUrl, supportedNftNetworks } from '../src/utils'
...
test('/tokenBalance', async () => {
    ...
    // mock every rpc url
    // const interceptables = supportedNftNetworks.map((network) => {
    //     const url = new URL(`https://eth-mainnet.g.alchemy.com/v2/fake_key`)
    //     return fetchMock.get(url.origin)
    // })

    const result = await worker.fetch(
        new Request('https://fake-cloudflare-worker-url.com/api/tokenBalance', {
            method: 'POST',
            headers: {
