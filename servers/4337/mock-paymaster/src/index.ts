...
const main = async () => {
	if (!process.env.BUNDLER_RPC) {
		throw new Error("BUNDLER_RPC is not set");
	}
	// verify we can connect to the bundler, if network unreachable then "fetch failed" error
	await fetch(process.env.BUNDLER_RPC);
