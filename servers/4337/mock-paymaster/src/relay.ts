...
// anvil + towns base contracts has an issue where the callGasLimit is too low
// so we need to set it to a higher value
// TODO: better fix
const anvilCallGasLimit = (gasLimit: bigint) => {
	return 10_000_000n
}
...
const handleMethodV06 = async (
	...
	if (estimateGas) {
		...
		op.callGasLimit = anvilCallGasLimit(op.callGasLimit);
...
const handleMethod = async (
	...
	// THIS STILL NEEDS WORK, DON"T USE IT YET
	if (parsedBody.method === 'alchemy_requestGasAndPaymasterAndData') {
		const params = alchemyGasRequestSchema.safeParse(
			parsedBody.params,
		);
		...
		const [{ userOperation, entryPoint, dummySignature }] = params.data;
		...
		if (entryPoint === ENTRYPOINT_ADDRESS_V06) {
			...
			} satisfies z.infer<typeof userOperationSchemaPaymasterV6>;
