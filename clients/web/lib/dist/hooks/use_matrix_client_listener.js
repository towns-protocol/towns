...
export function useMatrixClientListener() {
    const { accessToken, homeServer, isAuthenticated, userId, leaveRoom, setNewMessage, setRoom, setRoomName, setRooms, updateMembership, } = useStore();
    const matrixClientRef = useRef();
    const [syncRoomId, setSyncRoomId] = useState("");
    const [membershipChanged, setMembershipChanged] = useState("");
    useEffect(function () {
    ...
    const startClient = useCallback(async function () {
        if (accessToken && homeServer && userId) {
            ...
            async function (state, prevState, res) {
                if (state === "PREPARED") {
                ...
                switch (member.membership) {
                    case Membership.Invite: {
                        if (member.userId === client.getUserId()) {
                            updateMembership(member.roomId, member.userId, Membership.Invite);
                        }
                        break;
                    }
