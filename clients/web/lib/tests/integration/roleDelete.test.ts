...
import { getTestGatingNftAddress, NoopRuleData, Permission } from '@towns-protocol/web3'
...
describe('delete role', () => {
    test.skip('delete token-gated role with a channel using it', async () => {
        ...
        const newNftAddress = await getTestGatingNftAddress(alice.opts.baseChainId)
        if (!newNftAddress) {
