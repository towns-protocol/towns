...
describe.skip('unreadMessageCount', () => {
    ...
    test('create room, invite user, accept invite, and send message, check unread counts', async () => {
        ...
        await waitFor(() => expect(alice.getRoom(spaceId)?.getMyMembership()).toBe('join'))
        await waitFor(() => expect(alice.getRoom(channel_1)?.getMyMembership()).toBe('join'))
        ...
        await waitFor(() =>
            expect(alicesLastNotifications?.[channel_1.matrixRoomId]?.notification_count).toBe(2),
        )
        await waitFor(() =>
            expect(alicesLastNotifications?.[channel_2.matrixRoomId]?.notification_count).toBe(1),
        )
        // start clearing the notifications
        await alice.sendReadReceipt(channel_1)
        // and see the update
        await waitFor(() =>
            expect(alicesLastNotifications?.[channel_1.matrixRoomId]?.notification_count).toBe(0),
        )
        // clear
