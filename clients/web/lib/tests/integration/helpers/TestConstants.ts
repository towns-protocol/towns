...
import { TestGatingNFT } from '@towns-protocol/web3'
...
export class TestConstants {
    ...
    public static async getUnfundedWallet(): Promise<ethers.Wallet> {
        const tempWallet = ethers.Wallet.createRandom()
    ...
    public static async getWalletFromPrivateKey(privateKey: string): Promise<ethers.Wallet> {
        const provider = getJsonProvider()
        await provider.ready
