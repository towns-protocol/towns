...
export const useRegisterPasswordUser = () => {
    const { setDeviceId, setLoginError, setLoginStatus, setUserId, setUsername } = useMatrixStore()
    ...
    return useCallback(
        async (username: string, password: string): Promise<void> => {
            ...
            const response = await matrixRegisterUser(homeServerUrl ?? '', username, password)
...
async function matrixRegisterUser(
    ...
    try {
        ...
        const sessionId = ''
        const response = await newClient.register(username.toLowerCase(), password, sessionId, {
            type: 'm.login.dummy',
