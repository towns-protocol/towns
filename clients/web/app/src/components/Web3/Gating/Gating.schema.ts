import { z } from 'zod'
import { tokenEntitlementSchema, tokenSchema } from '@components/Tokens/TokenSelector/tokenSchemas'
...
export const gatingSchema = z
    .object({
        ...
        tokensGatedBy: z.array(tokenEntitlementSchema),
        clientTokensGatedBy: z.array(tokenSchema),
        usersGatedBy: z.array(z.string()),
    })
    .superRefine((data, ctx) => {
        if (data['gatingType'] === 'gated') {
            if (data['clientTokensGatedBy']?.length === 0 && data['usersGatedBy']?.length === 0) {
                ctx.addIssue({
                    code: z.ZodIssueCode.custom,
                    path: ['clientTokensGatedBy'],
