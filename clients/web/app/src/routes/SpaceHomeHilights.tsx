...
export const HomeHighlights = () => {
    ...
    const messages = useMemo(() => fakeMessages(spaceId?.slug ?? ''), [spaceId?.slug])
...
const useColumns = (messages: Message[], col: number) => {
    return useMemo(() => {
        ...
        return messages.reduce((columns: { id: string; messages: Message[] }[], m, i) => {
            ...
            <Stack justifyContent="center">
                {message.imageUrl && (
                    <Box grow position="relative" height="200">
                        <BackgroundImage padding src={message.imageUrl} justifyContent="end">
                            <Avatar size="avatar_md" src={user.avatarSrc} boxShadow="avatar" />
                        </BackgroundImage>
                    </Box>
                )}

                <Stack padding gap="md">
                    {!message.imageUrl && <Avatar size="avatar_md" src={user.avatarSrc} />}
