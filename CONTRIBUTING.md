...
- **Pnpm 10.x**: We're using pnpm to handle our monorepo. There should be only one `node_modules` and one lockfile at the root of the repository.
...
1. Run `pnpm install` from the root of the repository
...
- Run all unit tests via: `pnpm test:unit`
- Run all e2e tests via: `pnpm test:e2e`
...
e) you want a single script to run all tests within the package, add `"test: pnpm test:unit && pnpm test:e2e"` script to its package.json
