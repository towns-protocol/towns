...
- **pnpm**: We're using pnpm to handle our monorepo. There should be only one `node_modules` and one lockfile at the root of the repository.
...
1. Run `pnpm install && pnpm run prepare` from the root of the repository
...
- Run all unit tests via: `pnpm run test:unit`
- Run all e2e tests via: `pnpm run test:e2e`
- Run all tests (both unit and e2e) via: `pnpm test`
...
e) you want a single script to run all tests within the package, add `"test: pnpm run test:unit && pnpm run test:e2e"` script to its package.json
