...
if [ "$FROZEN" = "--frozen" ]; then
  if git diff --quiet --exit-code -p $DIFF_GLOB; then
    echo "No new types generated by build-town-types.sh"
  else
    echo "$(git diff -p $DIFF_GLOB)"
    echo "Error: build-town-types.sh generated new types with the --frozen flag. Please re-run ./scripts/build-town-types.sh to re-generate the files and commit the changes."
    exit 1
  fi
fi
