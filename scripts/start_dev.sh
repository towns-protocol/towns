...
commands=(
    "watch_sdk:cd packages/sdk && pnpm watch"
    "watch_encryption:cd packages/encryption && pnpm watch"
    "watch_dlog:cd packages/dlog && pnpm watch"
    "watch_proto:cd packages/proto && pnpm watch"
    "watch_web3:cd packages/web3 && pnpm watch"
    "watch_go:cd protocol && pnpm watch:go"
    ...
    "river_stream_metadata_multi_ne:pnpm --filter=@towns-protocol/stream-metadata dev:local_multi_ne"
)
...
if is_closed ; then
    ...
    pnpm --filter=@towns-protocol/stream-metadata kill:local_multi_ne
fi
