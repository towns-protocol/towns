...
git clean -fdx -e .DS_Store -e '.env.*' -e '.env.*.*' -e '.envrc' -e '*.yaml' -e '.env' -e .vscode -e '*.pem' -e '*.crt' -e '*.key' -e .keys -e *_key -e *_address -e .wrangler -e .dev.* -e test-config.json -e infra
