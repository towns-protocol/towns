...
echo "SKIP_LIMIT_VERIFICATION: ${SKIP_LIMIT_VERIFICATION}"
echo "OUTPUT_TO_FILE: ${OUTPUT_TO_FILE}"

if [ "$SKIP_LIMIT_VERIFICATION" = "false" ]; then
    echo "SKIP_LIMIT_VERIFICATION is false, running with random wallets"
else
    echo "SKIP_LIMIT_VERIFICATION is true, running as privy user."
fi
