...
usage() {
    ...
    echo "Prerequisites: you must ether have a .dev.vars file in ./servers/workers/stackup-worker or pass the following environment variables: LOCAL_PAYMASTER_RPC_URL, PRIVY_APP_KEY"
}
...
if [ -n "$LOCAL_PAYMASTER_RPC_URL" ]; then
  LOCAL_PAYMASTER_RPC_URL_OVERRIDE="$LOCAL_PAYMASTER_RPC_URL"
fi
...
if [ -n "$LOCAL_PAYMASTER_RPC_URL_OVERRIDE" ]; then
  LOCAL_PAYMASTER_RPC_URL="$LOCAL_PAYMASTER_RPC_URL_OVERRIDE"
fi

# if variables are not present, throw an error
if [ -z "$LOCAL_PAYMASTER_RPC_URL" ]; then
    echo "ERROR: Missing LOCAL_PAYMASTER_RPC_URL"
    exit 1
fi
...
echo "LOCAL_PAYMASTER_RPC_URL: ${LOCAL_PAYMASTER_RPC_URL}"
...
stackup_command="cd ./servers/workers/stackup-worker && yarn dev:local --var SKIP_PRIVY_VERIFICATION:$SKIP_PRIVY_VERIFICATION LOCAL_PAYMASTER_RPC_URL:$LOCAL_PAYMASTER_RPC_URL PRIVY_APP_KEY:$PRIVY_APP_KEY PRIVY_APP_ID:clml5fp7s013vmf0fnkabuaiw ALCHEMY_API_KEY:$ALCHEMY_API_KEY AUTH_SECRET:foo"
