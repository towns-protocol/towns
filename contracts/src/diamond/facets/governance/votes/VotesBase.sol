...
abstract contract VotesBase is EIP712Base, Nonces {
  ...
  function _getPastTotalSupply(
    ...
    require(timepoint < _clock(), "Votes: future lookup");
    return
      VotesStorage.layout()._totalCheckpoints.upperLookupRecent(SafeCastLib.toUint32(timepoint));
  }
    ...
    if (from != to && amount > 0) {
      if (from != address(0)) {
        (uint256 oldValue, uint256 newValue) = _push(
          VotesStorage.layout()._delegateCheckpoints[from], _subtract, SafeCastLib.toUint224(amount)
        );
        emit IVotes.DelegateVotesChanged(from, oldValue, newValue);
      }
      if (to != address(0)) {
        (uint256 oldValue, uint256 newValue) =
          _push(VotesStorage.layout()._delegateCheckpoints[to], _add, SafeCastLib.toUint224(amount));
