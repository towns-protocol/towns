...
abstract contract RewardsDistributionBase is IRewardsDistributionBase {
  ...
  function _currentSpaceDelegationReward(
    ...
    StakingRewards.Layout storage staking = RewardsDistributionStorage.layout().staking;
    address[] memory spaces = SpaceDelegationStorage.layout().spacesByOperator[operator].values();
    ...
    for (uint256 i; i < spaces.length; ++i) {
      ...
      unchecked {
        rewardPerTokenGrowth = currentRewardPerTokenAccumulated - treasure.rewardPerTokenAccumulated;
      }
      total +=
  ...
  function _revertIfNotOperatorOrSpace(
    address delegatee
  ) internal view {
  ...
  function _revertIfNotOperatorClaimer(
    address operator
  ) internal view {
    NodeOperatorStorage.Layout storage nos = NodeOperatorStorage.layout();
  ...
  function _revertIfPastDeadline(
    uint256 deadline
  ) internal view {
