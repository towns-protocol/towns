...
contract CouncilVault is Authorizable, IVotingVault {
  ...
  // The voting vaults that are approved to be used by the GSC
  ...
  /// @param _votingVaults The voting vaults that are approved to be used by the GSC
  ...
  function proveMembership(bytes[] calldata extraData) external {
    ...
    for (uint256 i = 0; i < votingVaults.length; i++) {
      ...
      uint256 votes = IVotingVault(votingVaults[i]).queryVotePower(
        msg.sender,
        block.number - 1,
        extraData[i]
      );
  ...
  function updateVotingVaults(address[] calldata _vaults) external onlyOwner {
    votingVaults = _vaults;
  }
}
