    ...
    ReentrancyGuardUpgradeable
{
    ...
    function nodeCrashExit(uint256 nodeHash) external nonReentrant {
        ...
        for (uint256 p; p < peersLength; ) {
            if (
                nodeHashToNode[nodeHash].peers[p].exitState !=
                PeerExitState.COMPLETE
            ) {
                nodeHashToNode[nodeHash].peers[p].exitState = PeerExitState
                    .ABANDONED;
            }
            unchecked {
                ++p;
        ...
        nodeHashToNode[nodeHash].balance =
            nodeHashToNode[nodeHash].balance /
            10;
    ...
    function nodeEvacuationComplete(
        ...
        for (uint256 p; p < peersLength; ) {
            if (exitingNode.peers[p].nodeHash == confirmingNodeHash) {
                ...
                nodeHashToNode[exitingNodeHash]
                    .peers[p]
                    .exitState = PeerExitState.COMPLETE;
            }
            if (
                nodeHashToNode[exitingNodeHash].peers[p].exitState ==
                PeerExitState.COMPLETE
        ...
        if (
            nodeHashToNode[exitingNodeHash].peers.length == completedEvacuation
        ) {
