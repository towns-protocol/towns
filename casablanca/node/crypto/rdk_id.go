...
func RdkIdFromPubKey(signerPubKey []byte) (RdkId, error) {
	...
	if len(signerPubKey) != 65 || (len(signerPubKey) < 1 && signerPubKey[0] != 4) {
		return RdkId{}, RiverError(protocol.Err_BAD_PUBLIC_KEY, "Invalid signer public key length", "len", len(signerPubKey))
	}
...
func RdkIdFromSlice(slice []byte) (RdkId, error) {
	if len(slice) != TOWNS_HASH_SIZE {
		return RdkId{}, RiverError(protocol.Err_BAD_PUBLIC_KEY, "Invalid slice length", "len", len(slice))
	}
