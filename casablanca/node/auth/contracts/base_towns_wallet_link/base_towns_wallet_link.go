...
func (_BaseTownsWalletLink *BaseTownsWalletLinkTransactor) LinkWalletToRootKey(opts *bind.TransactOpts, rootKey common.Address, rootKeySignature []byte, nonce uint64) (*types.Transaction, error) {
	return _BaseTownsWalletLink.contract.Transact(opts, "linkWalletToRootKey", rootKey, rootKeySignature, nonce)
}

// LinkWalletToRootKey is a paid mutator transaction binding the contract method 0xf5158913.
//
// Solidity: function linkWalletToRootKey(address rootKey, bytes rootKeySignature, uint64 nonce) returns()
func (_BaseTownsWalletLink *BaseTownsWalletLinkSession) LinkWalletToRootKey(rootKey common.Address, rootKeySignature []byte, nonce uint64) (*types.Transaction, error) {
	return _BaseTownsWalletLink.Contract.LinkWalletToRootKey(&_BaseTownsWalletLink.TransactOpts, rootKey, rootKeySignature, nonce)
}

// LinkWalletToRootKey is a paid mutator transaction binding the contract method 0xf5158913.
//
// Solidity: function linkWalletToRootKey(address rootKey, bytes rootKeySignature, uint64 nonce) returns()
func (_BaseTownsWalletLink *BaseTownsWalletLinkTransactorSession) LinkWalletToRootKey(rootKey common.Address, rootKeySignature []byte, nonce uint64) (*types.Transaction, error) {
	return _BaseTownsWalletLink.Contract.LinkWalletToRootKey(&_BaseTownsWalletLink.TransactOpts, rootKey, rootKeySignature, nonce)
}
...
func (_BaseTownsWalletLink *BaseTownsWalletLinkTransactor) RemoveLink(opts *bind.TransactOpts, wallet common.Address) (*types.Transaction, error) {
	return _BaseTownsWalletLink.contract.Transact(opts, "removeLink", wallet)
}

// RemoveLink is a paid mutator transaction binding the contract method 0x27a04d85.
//
// Solidity: function removeLink(address wallet) returns()
func (_BaseTownsWalletLink *BaseTownsWalletLinkSession) RemoveLink(wallet common.Address) (*types.Transaction, error) {
	return _BaseTownsWalletLink.Contract.RemoveLink(&_BaseTownsWalletLink.TransactOpts, wallet)
}

// RemoveLink is a paid mutator transaction binding the contract method 0x27a04d85.
//
// Solidity: function removeLink(address wallet) returns()
func (_BaseTownsWalletLink *BaseTownsWalletLinkTransactorSession) RemoveLink(wallet common.Address) (*types.Transaction, error) {
	return _BaseTownsWalletLink.Contract.RemoveLink(&_BaseTownsWalletLink.TransactOpts, wallet)
}
...
// BaseTownsWalletLinkRemoveLinkIterator is returned from FilterRemoveLink and is used to iterate over the raw logs and unpacked data for RemoveLink events raised by the BaseTownsWalletLink contract.
type BaseTownsWalletLinkRemoveLinkIterator struct {
	Event *BaseTownsWalletLinkRemoveLink // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *BaseTownsWalletLinkRemoveLinkIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(BaseTownsWalletLinkRemoveLink)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *BaseTownsWalletLinkRemoveLinkIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *BaseTownsWalletLinkRemoveLinkIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// BaseTownsWalletLinkRemoveLink represents a RemoveLink event raised by the BaseTownsWalletLink contract.
type BaseTownsWalletLinkRemoveLink struct {
	Wallet       common.Address
...
func (_BaseTownsWalletLink *BaseTownsWalletLinkFilterer) FilterRemoveLink(opts *bind.FilterOpts) (*BaseTownsWalletLinkRemoveLinkIterator, error) {

	logs, sub, err := _BaseTownsWalletLink.contract.FilterLogs(opts, "RemoveLink")
	if err != nil {
		return nil, err
	}
	return &BaseTownsWalletLinkRemoveLinkIterator{contract: _BaseTownsWalletLink.contract, event: "RemoveLink", logs: logs, sub: sub}, nil
}
...
//
// Solidity: event RemoveLink(address wallet, address secondWallet)
func (_BaseTownsWalletLink *BaseTownsWalletLinkFilterer) WatchRemoveLink(opts *bind.WatchOpts, sink chan<- *BaseTownsWalletLinkRemoveLink) (event.Subscription, error) {

	logs, sub, err := _BaseTownsWalletLink.contract.WatchLogs(opts, "RemoveLink")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(BaseTownsWalletLinkRemoveLink)
				if err := _BaseTownsWalletLink.contract.UnpackLog(event, "RemoveLink", log); err != nil {
					return err
				}
				event.Raw = log
...
//
// Solidity: event RemoveLink(address wallet, address secondWallet)
func (_BaseTownsWalletLink *BaseTownsWalletLinkFilterer) ParseRemoveLink(log types.Log) (*BaseTownsWalletLinkRemoveLink, error) {
	event := new(BaseTownsWalletLinkRemoveLink)
	if err := _BaseTownsWalletLink.contract.UnpackLog(event, "RemoveLink", log); err != nil {
		return nil, err
	}
