...
/* eslint-disable @typescript-eslint/no-non-null-assertion, @typescript-eslint/no-unsafe-return, @typescript-eslint/no-unused-vars */
...
export class LocalStorageCryptoStore extends MemoryCryptoStore {
    ...
    public constructor(private readonly store: Storage, private readonly userId: string) {
        super()
    }
    ...
    public async filterOutNotifiedErrorDevices(devices: IOlmDevice[]): Promise<IOlmDevice[]> {
        const notifiedErrorDevices =
            getJsonItem<MemoryCryptoStore['notifiedErrorDevices']>(
                this.store,
    ...
    public getSessionsNeedingBackup(limit: number): Promise<ISession[]> {
        const sessionsNeedingBackup =
            getJsonItem<string[]>(this.store, KEY_SESSIONS_NEEDING_BACKUP) || {}
    ...
    public deleteAllData(): Promise<void> {
        this.store.removeItem(`${KEY_END_TO_END_ACCOUNT}-${this.userId}`)
        return Promise.resolve()
    }
