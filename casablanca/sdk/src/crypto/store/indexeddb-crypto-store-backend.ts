...
export class Backend implements CryptoStore {
    ...
    public constructor(private db: IDBDatabase) {
        ...
        db.onversionchange = (): void => {
            log(`versionchange for indexeddb ${this.db.name}: closing`)
            db.close()
        }
    }
    ...
    public async unmarkSessionsNeedingBackup(
        sessions: ISession[],
        txn?: IDBTransaction,
    ): Promise<void> {
        if (!txn) {
    ...
    public async markSessionsNeedingBackup(
        sessions: ISession[],
        txn?: IDBTransaction,
    ): Promise<void> {
        if (!txn) {
...
export function upgradeDatabase(db: IDBDatabase, oldVersion: number): void {
    log(`Upgrading IndexedDBCryptoStore from version ${oldVersion}` + ` to ${VERSION}`)
    ...
    return new Promise((resolve, reject) => {
        ...
        txn.onerror = (event): void => {
            if ((txn as IWrappedIDBTransaction)._mx_abortexception !== undefined) {
                ...
                log('Error performing indexeddb txn', event)
                reject(txn.error)
            }
        }
