...
Towns Protocol is a permissionless, decentralized end-to-end encrypted chat 
network built on Base with an EVM-compatible L2 chain. The system enables the 
creation of programmable communication spaces with on-chain memberships and 
...
**Storage Layer** (`/core/node/storage/`): PostgreSQL-based persistent storage with a miniblock structure for efficient replication.

**Authentication & Authorization** (`/core/node/auth/`): Blockchain-based auth using Ethereum addresses, an entitlement system with rule-based permissions, and support for linked wallets.
...
- **All TypeScript files must pass Prettier formatting**  
  - Run `yarn prettier:fix` to automatically format all TypeScript files (checks and fixes formatting in one step)  
- **All PRs must pass global linting**  
  - Run `yarn lint` from the root directory before committing (ensures code quality and consistency across the repository)  
...
- Two development environments are available:  
  ...
  - `RUN_ENV=multi_ne` – Multi-node setup WITHOUT entitlement checks (faster and required for other SDK tests)  
- Local chains run on ports 8545 (Base) and 8546 (River)  
- PostgreSQL runs on port 5433 for local development  
- HTTPS certificates are managed via `generate-certs.sh`  
...
3. **Validation**: Ensured username uniqueness and allowed duplicate display names  
4. **Migration**: Created a migration to rename the existing column while preserving data  
5. **Tests**: Added explicit tests for duplicate display names, and made the bot test re-runnable with UUIDs  
6. **Frontend**: Updated all TypeScript references to use the new field names  
...
- **Do NOT** mention Claude, AI assistance, or any automated generation in commit messages  
