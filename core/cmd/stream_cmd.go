...
a specific node are included. Use 0 for max-streams to include all streams.`,
	...
	cmdStreamPlaceInitiate := &cobra.Command{
		Use:   "initiate",
		Short: "Initiate stream placement <wallet-file> <input-stream-file> <replication-factor>",
	...
	cmdStreamPlaceStatus := &cobra.Command{
		Use:   "status",
		Short: "Status stream placement <input-stream-file>",
	...
	cmdStreamPlaceEnterQuorum := &cobra.Command{
		Use:   "enter-quorum",
		Short: "Enter quorum <wallet-file> <input-stream-file>",
	...
	cmdStreamPlaceRemoveDroppedNodes := &cobra.Command{
		Use:   "drop-node",
		Short: "Remove ignored nodes from stream node list <wallet-file> <input-stream-file>",
		Long:  `Remove nodes that are ignored from the stream node list`,
		Args:  cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			return cmdStreamPlaceRemoveDroppedNodesCmd(cmdConfig, args)
		},
	}
	...
	cmdStreamPlace.AddCommand(cmdStreamPlaceRemoveDroppedNodes)
