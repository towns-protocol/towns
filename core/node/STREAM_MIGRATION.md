...
In the following example stream `0x13832fce7587c4c19f41d75f09c0d82c1bbf73abd8a90d594734acefb121de7c` is currently not replicated (replication factor=1) and managed by node `0xC6CF68A1BCD3B9285fe1d13c128953a14Dd1Bb60`. 

Nodes `0xB85dd7d21Bc093DAe6f1b56a20a45ec1770D11dD`, `0xDd5c1e1af10D47AC6859a494d7c1E4cb6a1DAB0d` and `0x553859AAC181c5568DaC867410698c586F3Cf1d3` are sync nodes in anticipation to join quorum.
```sh
...
```sh
./env/omega/run.sh stream not-migrated-all <output-directory>
```
...
> Important, migrating streams happens on a live system. Therefore the migration process needs to be paced to prevent the number of stream update transactions to fill up River Chain blocks or overwhelm stream nodes with reconcilation requests. This is done by submitting transactions to update the Stream Registry paced (wait for tx receipt to post the next transaction).
...
An example of manual intervention is to set the stream replication factor again with the same data causing a stream updated event that the node that is lagging uses as a trigger to reconcile again.
...
### Drop node from node list
For streams that are not just migrated to replicated streams but also away from an ignored node run the following command:

```sh
./env/omega/run.sh stream place drop-node \
	<wallet-file> \
	<outputfile-generated-in-the-enter-quorum-step>
```

The node has a hard coded list of nodes that are ignored and will drop it from the streams node list. The output file is in the same format as the input file with `.dropped_ignored_node` appended to it.