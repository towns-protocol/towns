...
func NewNotificationsStreamsTracker(
	...
	if err := tracker.StreamsTrackerImpl.Init(
		...
		trackingConfig,
		otelTracer,
		nil, // No placement listener for notifications
	); err != nil {
		return nil, err
	}
...
func (tracker *NotificationsStreamsTracker) TrackStream(stream *river.StreamWithId, isInit bool) (bool, *river.StreamWithId) {
	streamType := stream.StreamId().Type()
