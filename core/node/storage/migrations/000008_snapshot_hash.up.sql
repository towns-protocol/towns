DO $$
    ...
    BEGIN
        ...
        FOR i IN 0.. numPartitions LOOP
                ...
                -- Media streams
                EXECUTE 'ALTER TABLE miniblocks_m' || suffix || ' ADD COLUMN snapshot BYTEA;';
                EXECUTE 'ALTER TABLE miniblocks_m' || suffix || ' ALTER COLUMN snapshot SET STORAGE EXTERNAL;';
                EXECUTE 'ALTER TABLE miniblock_candidates_m' || suffix || ' ADD COLUMN snapshot BYTEA;';
                EXECUTE 'ALTER TABLE miniblock_candidates_m' || suffix || ' ALTER COLUMN snapshot SET STORAGE EXTERNAL;';

                -- Regular streams
                EXECUTE 'ALTER TABLE miniblocks_r' || suffix || ' ADD COLUMN snapshot BYTEA;';
                EXECUTE 'ALTER TABLE miniblocks_r' || suffix || ' ALTER COLUMN snapshot SET STORAGE EXTERNAL;';
                EXECUTE 'ALTER TABLE miniblock_candidates_r' || suffix || ' ADD COLUMN snapshot BYTEA;';
