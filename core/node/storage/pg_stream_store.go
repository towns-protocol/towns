...
func (s *PostgresStreamStore) readMiniblocksFromExternalStorageTx(
	ctx context.Context,
	tx pgx.Tx,
	streamId StreamId,
	fromInclusive int64,
...
func (s *PostgresStreamStore) readMiniblocksTx(
	...
	var sql string
	// if miniblock data is not stored in the database fetch the miniblock parts that help to find download
	// the miniblocks from external storage
	if lockStreamResult.MiniblockDataLocation != external.MiniblockDataStorageLocationDB {
		return s.readMiniblocksFromExternalStorageTx(ctx, tx, streamId, fromInclusive, toExclusive)
	} else if omitSnapshot {
...
func (s *PostgresStreamStore) getLastMiniblockNumberTx(
	...
	if maxSeqNum == 0 {
		...
		err := tx.QueryRow(
			ctx,
			s.sqlForStream(
				fmt.Sprintf("SELECT octet_length(blockdata) FROM %s WHERE stream_id = $1 AND seq_num = 0", table),
				streamID,
			),
			streamID,
		).Scan(&blockdataLength)
