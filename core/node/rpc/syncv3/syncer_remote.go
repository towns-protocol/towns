...
type remoteSyncer struct {
	...
	// log is the logger for the remote syncer.
	log *logging.Log
...
func NewRemoteSyncer(
	...
	log := logging.FromCtx(ctx).Named("syncv3-syncer-remote").With("addr", remoteAddr.Hex())
	...
	log = log.With("syncId", responseStream.Msg().SyncId)
	...
	if responseStream.Msg().GetSyncOp() != SyncOp_SYNC_NEW || responseStream.Msg().GetSyncId() == "" {
		...
		log.Errorw("Received unexpected sync stream message", "syncOp", responseStream.Msg().SyncOp)
...
func (s *remoteSyncer) Run() {
	...
	defer func() {
		if err := s.responseStream.Close(); err != nil {
			s.log.Errorw("Failed to close response stream", "remote", s.remoteAddr, "error", err)
		}
	}()
	...
	for s.responseStream.Receive() {
		...
		if res.GetSyncOp() == SyncOp_SYNC_UPDATE {
			...
			if err != nil {
				s.log.Errorw("Received invalid stream ID in sync update", "remote", s.remoteAddr, "error", err)
				continue
			}
			...
			if err = s.sendResponse(streamID, res); err != nil {
				...
				// TODO: Do not return, handle properly
				return
			}
		} else if res.GetSyncOp() == SyncOp_SYNC_DOWN {
			streamID, err := StreamIdFromBytes(res.GetStreamId())
			if err != nil {
				s.log.Errorw("Received invalid stream ID in sync down", "remote", s.remoteAddr, "error", err)
				continue
			}
	...
	if s.ctx.Err() == nil {
		s.log.Infow("remote node disconnected", "remote", s.remoteAddr)
	...
	for {
		...
		case <-pingTicker.C:
			...
			if lastMsgRecv.Add(recentActivityDeadline).Before(now) { // no recent activity -> conn dead
				s.log.Warnw("remote sync node time out", "remote", s.remoteAddr)
				s.cancel()
				return
			}
			...
			if _, err := s.client.PingSync(s.ctx, connect.NewRequest(&PingSyncRequest{
				...
				if !errors.Is(err, context.Canceled) {
					s.log.Errorw("ping sync failed", "remote", s.remoteAddr, "error", err)
				}
				// TODO: Retry few times before canceling the sync.
				s.cancel()
				return
			}
		case <-s.ctx.Done():
