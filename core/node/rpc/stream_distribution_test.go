...
func TestStreamsToNodeAssignment(t *testing.T) {
	...
	t.Run("Alpha", func(t *testing.T) {
		...
		testDistributionWithStateFromEnv(t, blockchain, chainConfig, riverContract)
	})
...
func testDistributionWithStateFromEnv(
	...
	for i := range streamsToAllocate {
		...
		nodes, err := distributor.ChooseStreamNodes(streamID, replFactor)
		require.NoError(err)
	...
	ensureStreamDistributionIsEven(require, distributorSim.NodeStreamCount())
}
...
func testStreamDistributionFromGreenState(t *testing.T) {
	...
	for range streamsToAllocate {
		...
		tt.require.Equal(replFactor, len(uniqueNodes), "expected stream to be place on repl factor unique nodes")
	}

	ensureStreamDistributionIsEven(tt.require, distributorSim.NodeStreamCount())
}
...
func ensureStreamDistributionIsEven(
	...
	for _, count := range nodeStreamCount {
		minStreams = min(minStreams, count)
		maxStreams = max(maxStreams, count)
	}

	// ensure that the min and max nodes don't have more than 5% difference in stream count
	require.GreaterOrEqual((105*minStreams)/100, maxStreams, "streams not evenly distributed")
}
