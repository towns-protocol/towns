...
type determineSnapshotsCase struct {
	name              string
	rangeStart        int64
	rangeEnd          int64
	snapshotSeqs      []int64
...
var determineSnapshotsCases = []determineSnapshotsCase{
	{
...
func TestDetermineSnapshotsToNullify(t *testing.T) {
	for _, tt := range determineSnapshotsCases {
		tt := tt
...
func TestDetermineSnapshotsToNullifyIdempotent(t *testing.T) {
	for _, tt := range determineSnapshotsCases {
		tt := tt
		t.Run(tt.name, func(t *testing.T) {
			first := DetermineStreamSnapshotsToNullify(
				tt.rangeStart,
				tt.rangeEnd,
				tt.snapshotSeqs,
				tt.retentionInterval,
			...
			second := DetermineStreamSnapshotsToNullify(
				tt.rangeStart,
				tt.rangeEnd,
				first,
				tt.retentionInterval,
