...
// NewMultiCores creates a zapcore.Core instance that combines multiple cores.
func NewMultiCores(cores ...zapcore.Core) MultiCores {
	mc := make(MultiCores, len(cores))
	copy(mc, cores)
	return mc
}

func (mc MultiCores) With(fields []zapcore.Field) zapcore.Core {
	clone := make(MultiCores, len(mc))
	for i := range mc {
...
func (mc MultiCores) Level() zapcore.Level {
	minLvl := zapcore.FatalLevel // mc is never empty
	for i := range mc {
...
func (mc MultiCores) Enabled(lvl zapcore.Level) bool {
	for i := range mc {
		if mc[i].Enabled(lvl) {
			return true
		}
...
func (mc MultiCores) Check(ent zapcore.Entry, ce *zapcore.CheckedEntry) *zapcore.CheckedEntry {
	for i := range mc {
...
func (mc MultiCores) Write(ent zapcore.Entry, fields []zapcore.Field) error {
	var err error
	for i := range mc {
...
func (c *CoreWithLevels) With(fields []zapcore.Field) zapcore.Core {
	newCore := *c
	newCore.Core = c.Core.With(fields)
	return &newCore
}

func (c *CoreWithLevels) Check(entry zapcore.Entry, ce *zapcore.CheckedEntry) *zapcore.CheckedEntry {
	if lvl, ok := c.Levels[Component(entry.LoggerName)]; ok {
		if lvl.Enabled(entry.Level) {
			return ce.AddCore(entry, c)
		}
		return ce
	}

	if def, ok := c.Levels[Default]; ok {
		if def.Enabled(entry.Level) {
			return ce.AddCore(entry, c)
		}
		return ce
	}
