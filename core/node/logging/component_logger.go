...
func (c *ComponentCore) Enabled(level zapcore.Level) bool {
	return true // always true since this hasn't got the context of the component
}
...
// component specific log levels, all comma separated.
// valid lines
...
func ParseLogLevels(cfg string) (LogLevels, error) {
	var (
		globalLogLevel = zapcore.InfoLevel
		tokens         = strings.Split(strings.ToLower(strings.ReplaceAll(cfg, " ", "")), ",")
		levels         = LogLevels{"common": zapcore.InfoLevel}
	)
	...
	if err := globalLogLevel.UnmarshalText([]byte(tokens[0])); err == nil {
		tokens = tokens[1:]
	}
	...
	for _, token := range tokens {
		...
		if err := lvl.UnmarshalText([]byte(parts[1])); err != nil {
			return levels, fmt.Errorf("invalid log configuration components value %s", cfg)
		}
