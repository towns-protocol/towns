...
// RateLimitConfig represents the complete rate limiting configuration
...
// ParsedRateLimitConfig contains the processed rate limiting configuration with parsed network ranges
type ParsedRateLimitConfig struct {
	*RateLimitConfig
	ExemptIPNets []*net.IPNet                   // Parsed exempt IP networks
	EndpointMap  map[string]EndpointLimitConfig // Quick lookup map
}

// ParseRateLimitConfig processes the raw rate limiting configuration into a more usable form
func ParseRateLimitConfig(config *RateLimitConfig) (*ParsedRateLimitConfig, error) {
	parsed := &ParsedRateLimitConfig{
