...
2. **Unbounded backlog for offline bots** – Unsigned events (no session key)
   ...
   backlog grows without bound, limited only by database capacity.
...
- We need a way to subscribe only to streams where bots are actually installed.
  A practical approach is to consume AppAccount events from the subgraph (or
  another Base indexer) so we can Add/Remove streams dynamically when bots join
  or leave channels.
