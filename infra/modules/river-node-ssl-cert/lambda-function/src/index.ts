...
async function updateCloudflareDNS(dnsRecord: DnsRecordWithoutPriority) {
    try {
        ...
        const cloudflare_token = await secretsManager.getSecretValue({
            SecretId: cloudflare_token_secret,
        })
...
async function cleanupCloudflareDNS() {
    try {
        ...
        const records = await cf.dnsRecords.browse(cloudflareZoneId, {
            type: 'TXT',
            name: challengeDnsRecordFQName,
        })
        ...
        console.log(
            'TXT record:',
            txtRecord,
            records.result?.map((record) => record.name),
        )
        if (!txtRecord) {
