...
export const createPrometheusConfig = async () => {
    ...
    const prometheusConfig = await fs.promises.readFile(
        PROMETHEUS_CONFIG_FILE_SOURCE,
        {
            encoding: 'utf8',
        },
    )
    await fs.promises.writeFile(
        PROMETHEUS_CONFIG_FILE_DESTINATION,
        prometheusConfig,
        {
            encoding: 'utf8',
        },
    ...
    console.info(
        `Prometheus config written to: ${PROMETHEUS_CONFIG_FILE_DESTINATION}`,
    )
}
