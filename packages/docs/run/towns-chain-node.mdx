...
With anvil running, we can deploy the [StreamRegistry](https://github.com/towns-protocol/towns/blob/main/packages/contracts/src/river/registry/StreamRegistry.sol) contract.

The easiest way to do so is by using the helper script, `deploy-river-registry.sh` found under `/towns/scripts`. This script uses a `make` target that uses `forge` to run the deployer script, which is a solidity file that inherits from the `Deployer` abstract contract (see [Deployer.s.sol](https://github.com/towns-protocol/towns/blob/main/packages/contracts/scripts/common/Deployer.s.sol)).
...
➜  towns git:(main) ✗ cast call $CONTRACT "getStreamsLength()(uint256)" --rpc-url http://localhost:8546
0
```
