---
title: "Upgrade Node"
description: "This guide demonstrates how to upgrade a Towns Stream Node on Towns Chain in Testnet. The Mainnet process follows the same steps, with different contract addresses and RPC URL for Towns Chain."
---

<Note>
  Before proceeding, ensure your node operator wallet is registered on Base and has been bridged to Towns Chain by the DAO. You'll need access to this wallet to manage node operations, including registration and fleet updates.
</Note>
...
<Note>
  It is currently reccommended to upgrade node software with existing node identity that is stored in RiverRegistry. Once Stream rebalancing is implemented, upgrades that create and register a new node identity can be safely enacted without potential stream downtime.
</Note>

This guide covers the docker upgrade path. We'll address source upgrades in future guides.
...
The following steps describe the reccommended path to upgrade Node FE as shown on the right side of the flowchart above.

<Note>
  This guide assumes the node operator performing the upgrade has access to an existing node's wallet private key as well as credentials to connect to a write instance of the Node Storage instance.
</Note>
...
<Steps>
  ...
  <Step title="Start Node FE">
    ...
    <Warning>
      Ensure you are starting your node's container with `SKIP_GENKEY=true` otherwise a new node wallet will be created and attached to your Node storage.
    </Warning>
