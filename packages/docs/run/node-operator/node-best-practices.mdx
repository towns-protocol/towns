...
<Step title="Backups">
    ...
    <Warning>Stream replication functionality is in development with basic replication factor support implemented. However, full peer recovery mechanisms may not be complete. Therefore, it is important that node operators backup their node storage regularly and ensure high availability through their cloud provider database setup until full replication is verified. </Warning>
