...
description: "Towns Stream Nodes can be run either remotely or locally based on operator preference. This guide shows you how to build and run a Towns Stream Node locally for development. We'll also cover diagnostic tools for monitoring the health of both local and remote nodes. Note that this guide assumes a standalone node not connected to the Towns protocol remote node registry on Base."
---

## Running a Towns Stream Node in Development

Running a Towns Stream Node from source is easy to do with [Go](https://go.dev/doc/install), [Docker](https://docs.docker.com/engine/install/), and [Foundry](https://github.com/foundry-rs/foundry) Ethereum toolkit installed. This tutorial guides you through running a Towns Stream Node with a Postgres backend.
...
Once you've installed Docker, Go, protobuf, Anvil, and buf, setup your environment as follows:
...
<Steps>
  ...
  <Step title="Upgrade Foundry toolkit">
    
  </Step>
  <Step title="Run yarn install">
    
  </Step>
  <Step title="Build protobufs">
    
  </Step>
  <Step title="Start Base Chain in anvil">
    
  </Step>
  <Step title="Start Towns Chain in anvil">
    
  </Step>
  <Step title="Configure stream nodes">
    
  </Step>
  <Step title="Start stream nodes with and without entitlements (on separate ports)">
    
  </Step>
  <Step title="Start xchain binary">
    
  </Step>
  <Step title="Start protobuf and typescript package watcher">
...
<Note>
  Before running a stream node on the remote testnet or mainnet networks, you must register it on Towns Chain. See onboarding instructions [**here**](https://docs.towns.com/run/node-operator/system-requirements-installation).
</Note>

### **Running a Stream Node in Info Mode for Diagnostics**
