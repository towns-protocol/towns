...
// Handle /ping command
bot.onSlashCommand('ping', async (ctx, event) => {
...
bot.onSlashCommand('weather', async (ctx, event) => {
  ...
  // ctx.args: ['San', 'Francisco']

  const location = ctx.args.join(' ')

  if (!location) {
    await ctx.send('Usage: /weather <location>\nExample: /weather San Francisco')
    return
  }

  const weather = await callWeatherAPI(location)
  await ctx.send(`Weather in ${location}: ${weather.temp}°F, ${weather.conditions}`)
})
```
...
```ts
// /greet @john @jane
bot.onSlashCommand('greet', async (ctx, event) => {
  if (ctx.mentions.length === 0) {
    await ctx.send('Usage: /greet @user1 @user2 ...')
    return
  }

  const greetings = ctx.mentions
    .map(m => `Hello <@${m.userId}>!`)
    .join('\n')

  await ctx.send(greetings, { mentions: ctx.mentions })
})
```
...
bot.onSlashCommand('ban', async (ctx, event) => {
  ...
  const isAdmin = await ctx.hasAdminPermission(ctx.userId, ctx.spaceId)
  if (!isAdmin) {
  ...
  try {
    ...
    await ctx.send(`✓ Banned <@${userToBan}>`)
  } catch {
...
bot.onSlashCommand('generate', async (ctx, event) => {
  // Payment already processed at this point
  await ctx.send('Thank you for your purchase! Here is your generated content...')
})
```
