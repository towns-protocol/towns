...
bot.onSlashCommand('setup-github', async (ctx, event) => {
...
bot.onSlashCommand('price-alerts', async (ctx, event) => {
  priceChannels.add(ctx.channelId)
  await ctx.send('Price alerts enabled')
})
...
// Rate limiting
bot.onSlashCommand('premium', async (ctx, event) => {
  const count = await redis.incr(`ratelimit:${ctx.userId}`)
