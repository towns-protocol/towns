...
```ts
import { hexToBytes, encodeFunctionData, erc20Abi, parseUnits } from 'viem'
import { getSmartAccountFromUserId } from '@towns-protocol/bot'

bot.onSlashCommand('sendTowns', async (handler, event) => {
  // Usage: /sendTowns @user 50
  const townsAddress = '0x00000000a22c618fd6b4d7e9a335c4b96b189a38' // Towns token
  ...
  await handler.sendInteractionRequest(
    event.channelId,
    {
      ...
      value: {
        id: 'towns-transfer',
        title: 'Send TOWNS',
        subtitle: `Send ${event.args[0]} TOWNS to ${mentionedUser.userId}`,
        content: {
          case: 'evm',
          value: {
            chainId: '8453',
