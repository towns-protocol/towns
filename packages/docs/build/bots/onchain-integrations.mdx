...
Get a free RPC URL from [Alchemy](https://www.alchemy.com/) or similar providers.
...
Handle transaction and signature responses:
  ...
  switch (response.payload.content?.case) {
    case 'transaction':
      ...
      if (txData.txHash) {
        ...
        await handler.sendMessage(
          event.channelId,
          `Transaction failed: ${txData.error}`
        )
      }
      break
...
| `chainId` | string | Chain ID (Base mainnet: `'8453'`) |
