...
## Event Listeners
All event listeners are asynchronous functions that receive a `ctx` (context) and an `event` object.

The `ctx` provides methods for sending messages and interacting with Towns, while the `event` contains the `BasePayload` and additional fields specific to the event type. You can access event data from either `ctx` or `event` (they contain the same payload data).
...
bot.onSlashCommand("video", async (ctx, event) => {
  const videoData = readFileSync('./video.mp4')

  await ctx.send("Check this out!", {
    attachments: [{
      type: 'chunked',
...
```ts
bot.onMessage(async (ctx, event) => {
  const shouldTip = checkIfDeserveTip(ctx.message)
  if (shouldTip) {
    await ctx.sendTip({
      userId: ctx.userId,
      amount: parseEther('0.001'),
      messageId: ctx.eventId,
...
bot.onSlashCommand('ban', async (ctx, event) => {
  ...
  const userToBan = ctx.mentions[0]?.userId
  if (userToBan) {
    try {
      await ctx.ban(userToBan, ctx.spaceId)
...
```ts
await ctx.ban(userId, spaceId)
```

### unban

```ts
await ctx.unban(userId, spaceId)
```
