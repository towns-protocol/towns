...
Messaging is the core application of Towns Protocol. Towns Stream Nodes serve stream events, which use protobufs to represent data. These stream events can contain both message-like events and validation metadata.

### Protocol Messaging Protobuf

The data schema of the Towns protocol can be defined declaratively by [protobuf.proto](https://github.com/river-build/river/blob/main/protocol/protocol.proto). Below is a simplified version of **protocol.proto** from January 2024, showing only the schema definitions relevant to messaging.
...
Channel messages enable group chat within Spaces. They are defined in `ChannelPayload` in `protocol.proto`. Each `ChannelPayload` message is typed as `EncryptedData`, which is a message that defines the payload of ciphertext.
...
Media files are chunked into byte arrays on nodes and support client-side pagination due to their potentially large size.