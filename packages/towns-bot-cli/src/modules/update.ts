...
import { getPackageManager, getInstallCommand, runCommand } from './utils.js'

interface PackageVersions {
  [key: string]: string
}

export async function update(_argv: any) {
  ...
  const packageJson = jsonc.parse(fs.readFileSync(packageJsonPath, 'utf-8'))
  const dependencies = { ...packageJson.dependencies, ...packageJson.devDependencies }
  
  const townsPackages = Object.keys(dependencies).filter(pkg => pkg.startsWith('@towns-protocol/'))
  ...
  if (edits.length > 0) {
    ...
    try {
      await runCommand(packageManager, [getInstallCommand(packageManager).split(' ')[1] || 'install'])
      console.log()
      console.log(green('✓'), 'Dependencies updated successfully!')
    } catch (error) {
      console.error(red('Error:'), 'Failed to install dependencies')
      console.log(yellow('Please run'), cyan(getInstallCommand(packageManager)), yellow('manually'))
    }
  } else {
