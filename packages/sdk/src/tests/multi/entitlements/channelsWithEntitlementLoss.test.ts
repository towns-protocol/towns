...
describe('channelsWithEntitlementLoss', () => {
    test('user booted on key request after entitlement loss', async () => {
        ...
        const tokenId = await TestERC721.publicMint(
            'TestNFT',
        ...
        await waitFor(() =>
            channelStream.view.membershipContent.isMember(
        ...
        await expect(
            alice.makeEventAndAddToStream(channelId!, payload),
        ...
        await waitFor(() =>
            expect(
                aliceUserStream.view.userContent.isMember(
                    channelId!,
