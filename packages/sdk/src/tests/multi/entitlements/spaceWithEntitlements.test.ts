...
describe('spaceWithEntitlements', () => {
    ...
    test('owner can ban other users', async () => {
        ...
        log('Bob kicks Alice')
        await expect(
            bob.removeUser(spaceId, alice.userId),
        ...
        await waitFor(() => {
            expect(
                aliceUserStreamView.userContent.isMember(
                    spaceId,
                    MembershipOp.SO_JOIN,
                ),
            ).toBe(false)
    ...
    test('user with banning permission can ban other users', async () => {
        ...
        await expect(alice.removeUser(spaceId, carol.userId)).rejects.toThrow(
            /7:PERMISSION_DENIED/,
        )
        ...
        await waitFor(() => {
            expect(
                carolUserStreamView.userContent.isMember(
                    spaceId,
                    MembershipOp.SO_JOIN,
                ),
            ).toBe(true)
        ...
        await expect(
            alice.removeUser(spaceId, carol.userId),
        ).resolves.not.toThrow()
