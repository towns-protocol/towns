...
describe('syncAgent.test.ts', () => {
    ...
    test('syncAgent', async () => {
        ...
        expect(syncAgent.riverConnection.authStatus.value).toBe(
        ...
        await waitFor(() =>
            expect(syncAgent.riverConnection.authStatus.value).toBe(
                AuthStatus.Credentialed,
            ),
        )
        expect(
            Object.keys(syncAgent.user.memberships.data.memberships).length,
        ).toBe(0)
        ...
        expect(syncAgent.user.memberships.data.memberships[spaceId].op).toBe(
            MembershipOp.SO_JOIN,
        )
        expect(
            syncAgent.user.memberships.data.memberships[defaultChannelId].op,
        ).toBe(MembershipOp.SO_JOIN)
        expect(syncAgent.riverConnection.authStatus.value).toBe(
            AuthStatus.ConnectedToRiver,
        )
