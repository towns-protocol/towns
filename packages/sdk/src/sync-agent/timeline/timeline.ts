...
export class MessageTimeline {
        ...
        this.unsubFn = stream.view.streamsView.timelinesView.subscribe(
            (state: TimelinesViewModel) => {
                this.events.setValue(
                    state.timelines[this.streamId] ?? EMPTY_TIMELINE,
                )
                this.threads.setValue(
                    state.threads[this.streamId] ?? EMPTY_RECORD,
                )
                this.threadsStats.setValue(
                    state.threadsStats[this.streamId] ?? EMPTY_RECORD,
                )
                this.reactions.setValue(
                    state.reactions[this.streamId] ?? EMPTY_RECORD,
                )
    ...
    async scrollback(): Promise<{
        terminus: boolean
        fromInclusiveMiniblockNum: bigint
    }> {
        return this.riverConnection.callWithStream(
            this.streamId,
            async (client) => {
                return client
