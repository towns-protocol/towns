...
library TippingMod {
    ...
    function sendTip(
        ...
        bytes calldata data
    ) internal returns (uint256 protocolFee, uint256 tipAmount) {
    ...
    function emitTipMember(MembershipTipParams memory req, uint256 tipAmount) internal {
    ...
    function depositTip(address self, address currency, uint256 amount) internal {
    ...
    function getTipsByWallet(address wallet, address currency) internal view returns (uint256) {
        return getStorage().tippingStatsByCurrencyByWallet[wallet][currency].tipAmount;
    }

    function getTipCountByWallet(address wallet, address currency) internal view returns (uint256) {
        return getStorage().tippingStatsByCurrencyByWallet[wallet][currency].totalTips;
    }

    function getTipsByTokenId(uint256 tokenId, address currency) internal view returns (uint256) {
        return getStorage().tipsByCurrencyByTokenId[tokenId][currency];
    }

    function getTippingCurrencies() internal view returns (address[] memory) {
        return getStorage().currencies.values();
    }

    function getTotalTipsByCurrency(address currency) internal view returns (uint256) {
        return getStorage().tippingStatsByCurrency[currency].totalTips;
    }

    function getTipAmountByCurrency(address currency) internal view returns (uint256) {
