...
abstract contract MembershipTiersBase is IMembershipTiersBase, MembershipBase, ERC721ABase {
    ...
    function _getTierRenewalPrice(
        uint256 tokenId,
        uint16 tierId
    ) internal view returns (uint256 tierRenewalPrice) {
        if (tierId == 0) return _getMembershipRenewalPrice(tokenId, _totalSupply());
        tierRenewalPrice = MembershipStorage.layout().renewalPriceByTokenId[tokenId];
        IPlatformRequirements platformReqs = _getPlatformRequirements();
        uint256 minPrice = platformReqs.getMembershipMinPrice();
        if (tierRenewalPrice < minPrice) return platformReqs.getMembershipFee();
        return tierRenewalPrice;
    }
