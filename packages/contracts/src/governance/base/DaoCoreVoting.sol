...
/// @title DaoCoreVoting
/// @dev Modifies CoreVoting to only proposals that have been approved by the GSC
contract DaoCoreVoting is Authorizable, ReentrancyBlock, ICoreVoting {
  ...
  function proposal(
    ...
    // the GSC (governance steering comity) is the only contract that can submit proposals
    require(isAuthorized(msg.sender), "proposals must be submitted by GSC");
