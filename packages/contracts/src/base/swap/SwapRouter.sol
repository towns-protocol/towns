...
contract SwapRouter is ReentrancyGuardTransient, ISwapRouter, Facet {
    ...
    function executeSwap(
        ...
        uint256 finalAmount;
        {
            (uint256 posterFee, uint256 treasuryFee) = _collectFees(tokenOut, amountOut, poster);
            finalAmount = amountOut - posterFee - treasuryFee;
        }

        // Transfer remaining tokens to user
        CurrencyTransfer.transferCurrency(tokenOut, address(this), msg.sender, finalAmount);
