...
library DeployMembershipToken {
    using DynamicArrayLib for DynamicArrayLib.DynamicArray;

    function selectorsExceptTokenURI() internal pure returns (bytes4[] memory res) {
    ...
    function makeCut(
        ...
        return IDiamond.FacetCut(facetAddress, action, selectorsExceptTokenURI());
    }

    function deploy() internal returns (address) {
        return LibDeploy.deployCode("MembershipToken.sol", "");
    }
}
