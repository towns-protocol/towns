...
// interfaces
import {IEntitlements, IEntitlementsStructs} from "contracts/src/towns/facets/entitlements/IEntitlements.sol";
...
contract EntitlementsTest is TownTest, IEntitlementsStructs {
  ...
  function setUp() public override {
    super.setUp();
    entitlements = IEntitlements(town);
  ...
  function test_addImmutableEntitlements_revert_when_not_owner() external {
    ...
    vm.prank(user);
    vm.expectRevert(
      abi.encodeWithSelector(TokenOwnable__NotOwner.selector, user)
    );
    entitlements.addImmutableEntitlements(immutableEntitlements);
  }
