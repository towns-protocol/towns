...
export async function fetchSpaceMemberMetadata(
    ...
    try {
        const metadata = await getSpaceMemberMetadata(
            logger,
            spaceAddress,
            tokenId,
        )
        ...
        logger.error(
            { spaceAddress, tokenId, err: error },
            'Failed to fetch space contract info',
        )
        return reply
            .code(404)
            .header('Cache-Control', CACHE_CONTROL['4xx'])
...
const getSpaceMemberMetadata = async (
    ...
    const [name, renewalPrice, membershipExpiration, isBanned] =
        await Promise.all([
            spaceDapp.spaceOwner
                .getSpaceInfo(spaceAddress)
                .then((spaceInfo) => spaceInfo.name),
