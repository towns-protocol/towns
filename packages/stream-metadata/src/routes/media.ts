...
export async function fetchMedia(request: FastifyRequest, reply: FastifyReply) {
    ...
    if (!paramsResult.success) {
        const errorMessage =
            paramsResult.error?.errors[0]?.message || 'Invalid parameters'
    ...
    if (!queryResult.success) {
        const errorMessage =
            queryResult.error?.errors[0]?.message || 'Invalid parameters'
        logger.info(errorMessage)
        return reply
            .code(400)
            .header('Cache-Control', CACHE_CONTROL['4xx'])
    ...
    try {
        const mediaContent = await getMediaStreamContent(
            logger,
            mediaStreamId,
            key,
            iv,
        ...
        logger.error(
            { mediaStreamId, err: error },
            'Failed to fetch media stream content',
        )
        ...
        return reply
            ...
            .send({
                error: 'Not Found',
                message: 'Failed to fetch media stream content',
            })
    }
}
