...
describe('integration/stream-metadata/space/:spaceAddress', () => {
    ...
    async function runDecriptionTest(
        ...
        const tx = await spaceDapp.createLegacySpace(
            createSpaceParams,
            provider.signer,
        )
        ...
        const response = await axios.get<SpaceMetadataResponse>(
            `${baseURL}/${route}`,
        )
    ...
    async function runSpaceImageTest(spaceUri: string) {
         ...
         */
        const { streamId: spaceStreamId } =
            await bobsClient.createSpace(spaceAddress)
        expect(spaceStreamId).toBeDefined()
        ...
        const { eventId } = await bobsClient.setSpaceImage(
            spaceStreamId,
            chunkedMedia,
        )
        ...
        try {
            response = await axios.get<SpaceMetadataResponse>(
                `${baseURL}/${route}`,
                {
            ...
            if (
                ...
                response =
                    error.response as AxiosResponse<SpaceMetadataResponse> // Capture the 302 response
            } else {
    ...
    it('should return 404 /space', async () => {
        ...
        try {
            ...
            throw new Error(
                `Expected request to fail with status ${expectedStatus})`,
            )
        } catch (error) {
    ...
    it('should return status 200 - both descriptions have values', async () => {
        await runDecriptionTest(
            'bobs space short description',
            'bobs space long description',
        )
    })
