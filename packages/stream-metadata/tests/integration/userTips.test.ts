...
describe('integration/stream-metadata/userTips', () => {
    ...
    test.skip('should return correct tip counts after a user sends a tip', async () => {
        ...
        const aliceResponse = await axios.get<UserTipsResponsePayload>(
            `${baseURL}/${aliceRoute}`,
        )

        expect(aliceResponse.status).toBe(200)
        expect(aliceResponse.headers['content-type']).toContain(
            'application/json',
        )
        expect(aliceResponse.data.tips.received[ETH_ADDRESS]).toEqual(
            tipAmount.toString(),
        )
        ...
        expect(aliceResponse.data.tips.sentCount[ETH_ADDRESS]).toEqual(
            undefined,
        )
        ...
        const bobResponse = await axios.get<UserTipsResponsePayload>(
            `${baseURL}/${bobRoute}`,
        )

        expect(bobResponse.status).toBe(200)
        expect(bobResponse.headers['content-type']).toContain(
            'application/json',
        )
        expect(bobResponse.data.tips.sent[ETH_ADDRESS]).toEqual(
            tipAmount.toString(),
        )
        ...
        expect(bobResponse.data.tips.receivedCount[ETH_ADDRESS]).toEqual(
            undefined,
        )
    ...
    test('should return 404 for a non-existent user', async () => {
        ...
        log(
            `Fetching tips for non-existent user ${fakeUserId} from ${baseURL}/${route}`,
        )
    ...
    test('should return 400 for an invalid userId format', async () => {
        ...
        log(
            `Fetching tips for invalid userId ${invalidUserId} from ${baseURL}/${route}`,
        )
