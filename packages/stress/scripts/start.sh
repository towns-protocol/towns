...
export APP_REGISTRY_ADDRESS="${APP_REGISTRY_ADDRESS:-}" # not required if using deployment in packages/generated/deployments
...
for i in $(seq $PROCESS_START_INDEX $((PROCESS_END_INDEX - 1))); do  # seq is inclusive
  PROCESS_INDEX=$i CLIENTS_PER_PROCESS=$CLIENTS_PER_PROCESS bun run start &
  pids+=($!)
done
