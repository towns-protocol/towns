...
it('cryptoAesGcm', async () => {
    ...
    const decrypted16 = await decryptAesGcm(
        key,
    ...
    const decrypted1000 = await decryptAesGcm(
        key,
        encrypted1000.ciphertext,
        encrypted1000.iv,
    )
    ...
    const decrypted1000a = await decryptAesGcm(
        key,
        encrypted1000a.ciphertext,
        encrypted1000a.iv,
    )
    expect(decrypted1000a).toEqual(data1000)

    // Check key2 works for decrypting
    const decrypted1000c = await decryptAesGcm(
        key2,
        encrypted1000a.ciphertext,
        encrypted1000a.iv,
    )
    ...
    const decrypted1000d = await decryptAesGcm(
        key,
        encrypted1000d.ciphertext,
        encrypted1000d.iv,
    )
    ...
    await expect(
        decryptAesGcm(key, encrypted1a, encrypted1.iv),
    ).rejects.toThrow()
    ...
    await expect(
        decryptAesGcm(key, encrypted1000b, encrypted1000.iv),
    ).rejects.toThrow()
    ...
    await expect(
        decryptAesGcm(key, encrypted1000c, encrypted1000.iv),
    ).rejects.toThrow()
    ...
    await expect(
        decryptAesGcm(key, encrypted1000.ciphertext, modifiedIv),
    ).rejects.toThrow()
    ...
    await expect(
        decryptAesGcm(modifiedKey, encrypted1.ciphertext, encrypted1.iv),
    ).rejects.toThrow()
    await expect(
