...
export const GdmTimelineRoute = () => {
  ...
  const formatMemberNames = useCallback(() => {
    ...
    const otherUserIds = members.userIds.filter(
    ...
    if (otherUserIds.length <= 2) {
      ...
      const firstMember = sync.gdms
        .getGdm(gdmStreamId!)
        .members.get(otherUserIds[0]);
      const firstMemberData = firstMember.data;
      const firstName =
        firstMemberData.displayName ||
        firstMemberData.username ||
        shortenAddress(otherUserIds[0]);

      const secondMember = sync.gdms
        .getGdm(gdmStreamId!)
        .members.get(otherUserIds[1]);
      const secondMemberData = secondMember.data;
      const secondName =
        secondMemberData.displayName ||
        secondMemberData.username ||
        shortenAddress(otherUserIds[1]);
